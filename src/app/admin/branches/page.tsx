...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      let gHtml = html;',
      '      const searchIdx = gHtml.indexOf(\'id="search"\');',
      '      const rsoIdx = gHtml.indexOf(\'id="rso"\');',
      '      let cutIdx = -1;',
      '      if (searchIdx > 0) cutIdx = searchIdx;',
      '      if (rsoIdx > 0 && (cutIdx === -1 || rsoIdx < cutIdx)) cutIdx = rsoIdx;',
      '      if (cutIdx > 0) gHtml = gHtml.substring(0, cutIdx);',
      '',
      '      const rMatch = gHtml.match(/<span[^>]*>([0-5][.,]\\\\d)<\\/span>[^<]*<a[^>]*>/i) || gHtml.match(/Оценка:\\\\s*([0-5][.,]\\\\d)/i) || gHtml.match(/aria-label="Рейтинг ([0-5][.,]\\\\d) из 5/i) || gHtml.match(/>([0-5][.,]\\\\d)<\\/span>/);',
      '      const cMatch = gHtml.match(/>([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:отзыв|отзыва|отзывов)/i) || gHtml.match(/aria-label="[^"]*?([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:отзыв|отзыва|отзывов)/i) || gHtml.match(/<a[^>]*>([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:Google|отзыв)/i) || gHtml.match(/<span>\\\\(([\\\\d\\\\s\\\\u00A0]+)\\\\)<\\/span>/i) || gHtml.match(/\\\\(([\\\\d\\\\s\\\\u00A0]+)\\\\)/);',
      '      if (rMatch) rating = parseFloat(rMatch[1].replace(",", "."));',
      '      if (cMatch) count = parseInt(cMatch[1].replace(/[^\\\\d]/g, ""));',
      '    }',
      '',
      '    if (!rating) {',
