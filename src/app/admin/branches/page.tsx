...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '  try {',
      '    // ==========================================',
      '    // ЛОГИКА ДЛЯ GOOGLE (SERPER.DEV)',
      '    // Два режима — выбираются автоматически по форме googleSearchQuery:',
      '    //   1) CID-URL ("https://www.google.com/maps?cid=NNN") →',
      '    //      /search с запросом "cid:NNN", парсим knowledgeGraph.',
      '    //      Однозначно открывает именно нужную карточку.',
      '    //   2) Любой другой текст (имя филиала, адрес и т.п.) →',
      '    //      /places с этим текстом, как и раньше. Подходит для городов',
      '    //      где нет конкурентов с похожими названиями.',
      '    // ==========================================',
      '    if (service === "google" && serperKey) {',
      '      console.log("⏳ [GOOGLE] Отправка запроса в Serper API...");',
      '      ',
      '      const cidMatch = String(urlOrQuery).match(/[?&]cid=(\\d+)/);',
      '      const useCidSearch = !!cidMatch;',
      '      const apiUrlSerper = useCidSearch',
      '        ? "https://google.serper.dev/search"',
      '        : "https://google.serper.dev/places";',
      '      const queryString = useCidSearch ? ("cid:" + cidMatch[1]) : urlOrQuery;',
      '      const payload = { "q": queryString, "gl": "ru", "hl": "ru" };',
      '      ',
      '      const options = {',
      '        method: "post",',
      ...
      '        ',
      '        if (useCidSearch) {',
      '          // /search: rating и ratingCount лежат в knowledgeGraph.',
      '          const kg = json.knowledgeGraph;',
      '          if (kg && kg.rating) {',
      '            rating = parseFloat(kg.rating);',
      '            count = parseInt(kg.ratingCount || kg.reviews || 0);',
      '            isSuccess = true;',
      '            console.log("⚡ [GOOGLE] Успех (cid)! Данные получены: " + (kg.title || queryString));',
      '          } else {',
      '            console.warn("❌ [GOOGLE] knowledgeGraph пуст или без рейтинга для " + queryString);',
      '          }',
      '        } else if (json.places && json.places.length > 0) {',
      '          const place = json.places[0];',
      '          if (place.rating) {',
      '            rating = parseFloat(place.rating);',
      '            count = parseInt(place.ratingCount || 0);',
      '            isSuccess = true;',
      '            console.log("⚡ [GOOGLE] Успех! Данные получены: " + place.title);',
      '          } else {',
      '            console.warn("❌ [GOOGLE] Компания найдена, но у неё нет рейтинга.");',
      '          }',
      '        } else {',
      '          console.warn("❌ [GOOGLE] Serper API не нашел компанию по запросу: " + queryString);',
      '        }',
      '      } else {',
