...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '',
      '    if (rating > 0) {',
      '      console.log("--- Проверка связи (" + service + ") ---");',
      '      const testGet = UrlFetchApp.fetch(apiUrl, { method: "GET", muteHttpExceptions: true });',
      '      console.log("Статус GET (связь): " + testGet.getResponseCode());',
      '      ',
      '      if (testGet.getResponseCode() !== 200) {',
      '        console.error("❌ Сайт недоступен или блокирует запросы (Код " + testGet.getResponseCode() + ")");',
      '        return;',
      '      }',
      '',
      '      console.log("✅ Связь установлена. Извлечение: " + rating + " (" + count + ")");',
      '      try {',
      ...
      '        });',
      '        ',
      '        const code = pushResponse.getResponseCode();',
      '        const body = pushResponse.getContentText();',
      '        ',
      '        if (code === 200) {',
      '          console.log("✅ Данные успешно загружены на сайт (" + service + ")");',
      '        } else {',
      '          console.error("❌ Ошибка загрузки (" + service + "): Код " + code + ". Ответ сервера: " + body.substring(0, 100));',
      '        }',
      '      } catch (e) {',
      '        console.error("❌ Сбой сети при отправке (" + service + "): " + e.message);',
      '      }',
      '    } else {',
