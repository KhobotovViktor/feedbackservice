...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      const rMatch = html.match(/([0-9][,.][0-9])\\s*из\\s*5/) || html.match(/([0-9][,.][0-9])\\s*stars/) || html.match(/ratingValue["\']>([0-9.]+)/) || html.match(/data-score=["\']([0-9.]+)/) || html.match(/aria-label="Рейтинг\s*([0-9.]+)/);',
      '      const cMatch = html.match(/(\\d+)\\s*(?:отзыв|отзывов|отзыва|reviews)/) || html.match(/ratingCount["\']>(\\d+)/) || html.match(/data-count=["\'](\\d+)/) || html.match(/([0-9\s]+) Google reviews/);',
      '      if (rMatch) rating = rMatch[1].replace(",", ".");',
      '      if (cMatch) count = cMatch[1].replace(/\D/g, "");',
      '    } else if (service === "2gis") {',
      '      const rMatch = html.match(/"ratingValue["\']:\\s*["\']?([0-9.]+)/) || html.match(/>([0-9][,.][0-9])<\/span>/) || html.match(/span\s*class=["\']?_y10azs["\']?>([0-9.]+)/);',
      '      const cMatch = html.match(/"reviewCount["\']:\\s*["\']?(\\d+)/) || html.match(/>(\\d+)\s*отзыв/) || html.match(/span\s*class=["\']?_jspzdm["\']?>(\d+)/);',
      '      if (rMatch) rating = rMatch[1] ? rMatch[1].replace(",", ".") : null;',
      '      if (cMatch) count = cMatch[1] ? cMatch[1].toString().replace(/\D/g, "") : null;',
      '    }',
      '',
      '    if (!rating) {',
      ...
      '      console.warn("⚠️ API заблокирован Vercel. Пробую через ScraperAPI...");',
