...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      ',
      '      const cidMatch = String(urlOrQuery).match(/[?&]cid=(\\d+)/);',
      '      const targetCid = cidMatch ? cidMatch[1] : null;',
      '      // Когда у нас есть CID-URL — Serper /places такой URL текстом не',
      '      // понимает. Используем имя филиала как текстовый запрос и потом',
      '      // фильтруем выдачу по cid.',
      '      const textQuery = targetCid ? (branchName || String(urlOrQuery)) : String(urlOrQuery);',
