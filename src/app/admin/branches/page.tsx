...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      finalUrl = "http://api.scraperapi.com/?api_key=" + scraperKey + "&premium=true&country_code=ru&url=" + encodeURIComponent(url);',
      '    } else if (service === "2gis" && scraperKey) {',
      '      finalUrl = "http://api.scraperapi.com/?api_key=" + scraperKey + "&render=true&country_code=ru&url=" + encodeURIComponent(url);',
      '    }',
      '',
      '    let response;',
      '    let retries = 2;',
      ...
      '      console.warn("❌ [" + service + "] Ошибка извлечения: Код " + response.getResponseCode());',
      '      console.warn("Причина: " + response.getContentText().substring(0, 300));',
      '      return;',
      '    }',
      '',
      ...
      '',
      '    if (service === "yandex") {',
      '      const rMatch = html.match(/itemprop="ratingValue" content="([\\\\d.]+)"/i) || html.match(/class="business-rating-badge-view__rating-text[^>]*>([\\\\d,.]+)/i) || html.match(/rating-text">([\\\\d,.]+)/i) || html.match(/class="Rating-Value">([\\\\d,.]+)/i);',
      '      const cMatch = html.match(/itemprop="reviewCount" content="(\\\\d+)"/i) || html.match(/class="business-header-rating-view__text[^>]*>([\\\\d\\\\s]+)\\\\s*(?:оцен|отзыв)/i) || html.match(/class="business-rating-amount-view[^>]*>([\\\\d\\\\s]+)\\\\s*(?:оцен|отзыв)/i) || html.match(/aria-label="([\\\\d\\\\s]+)\\\\s+(?:оцен|отзыв)/i);',
      '      if (rMatch) rating = parseFloat(rMatch[1].replace(",", "."));',
      '      if (cMatch) count = parseInt(cMatch[1].replace(/\\\\s/g, ""));',
      '    } else if (service === "2gis") {',
      '      const rMatch = html.match(/class="_y10azs">([\\\\d.]+)/i);',
      '      const cMatch = html.match(/class="_jspzdm">(\\\\d+)\\\\s+оцен/i);',
      '      if (rMatch) rating = parseFloat(rMatch[1]);',
      '      if (cMatch) count = parseInt(cMatch[1]);',
      '    } else if (service === "google") {',
      '      let gBlock = html.match(/([0-5][.,]\\\\d)<\\/span>[^<]*<a[^>]*data-async-trigger="reviewDialog"[^>]*>([\\\\d\\\\s\\\\u00A0]+)\\\\s*отзыв/i);',
      '      if (!gBlock) gBlock = html.match(/<span[^>]*aria-hidden="true"[^>]*>([0-5][.,]\\\\d)<\\/span>[\\\\s\\\\S]{0,150}?<span>\\\\(([\\\\d\\\\s\\\\u00A0]+)\\\\)<\\/span>/i);',
      '      if (!gBlock) gBlock = html.match(/([0-5][.,]\\\\d)[\\\\s\\\\S]{0,150}?<a[^>]*href="[^"]*#lrd=[^"]*"[^>]*>([\\\\d\\\\s\\\\u00A0]+)\\\\s*отзыв/i);',
      '      if (!gBlock) gBlock = html.match(/([0-5][.,]\\\\d)[\\\\s\\\\S]{0,150}?([\\\\d\\\\s\\\\u00A0]+)\\\\s*отзыв\\\\S*\\\\s*(?:в\\\\s+Google|Google\\\\s+reviews)/i);',
      '',
      '      if (!gBlock) {',
      '         const broadMatches = [...html.matchAll(/([0-5][.,]\\\\d)[\\\\s\\\\S]{1,150}?([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:отзыв|Google)/gi)];',
      '         for (let m of broadMatches) {',
      '            const ctx = html.substring(Math.max(0, m.index - 250), Math.min(html.length, m.index + 250));',
      '            if (!ctx.match(/yandex|2gis|zoon/i)) {',
      '               rating = parseFloat(m[1].replace(",", "."));',
      '               count = parseInt(m[2].replace(/[^\\\\d]/g, ""));',
      '               break;',
      '            }',
      '         }',
      '      }',
      '      if (gBlock && !rating) {',
      '        rating = parseFloat(gBlock[1].replace(",", "."));',
      '        count = parseInt(gBlock[2].replace(/[^\\\\d]/g, ""));',
      '      }',
      '    }',
      '',
      '    if (!rating) {',
