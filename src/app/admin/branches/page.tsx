...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      const gBlock = html.match(/([0-5][.,]\\\\d)[\\\\s\\\\S]{0,250}?([\\\\d\\\\s\\\\u00A0]*\\\\d)\\\\s*отзыв\\\\S*\\\\s*(?:в\\\\s+Google|Google)/i);',
      '      if (gBlock) {',
      '        rating = parseFloat(gBlock[1].replace(",", "."));',
      '        count = parseInt(gBlock[2].replace(/[^\\\\d]/g, ""));',
      '      } else {',
      '        const nRMatch = html.match(/<span[^>]*aria-hidden="true"[^>]*>([0-5][.,]\\\\d)<\\/span>/) || html.match(/<span[^>]*>([0-5][.,]\\\\d)<\\/span>\\\\s*<span[^>]*aria-label="[^"]*отзыв/);',
      '        const nCMatch = html.match(/<span[^>]*aria-label="([\\\\d\\\\s\\\\u00A0]+)\\\\s*отзыв/);',
      '        if (nRMatch) rating = parseFloat(nRMatch[1].replace(",", "."));',
      '        if (nCMatch) count = parseInt(nCMatch[1].replace(/[^\\\\d]/g, ""));',
      '      }',
      '    }',
      '',
      '    if (!rating) {',
