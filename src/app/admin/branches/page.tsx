...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      const rMatch = html.match(/<span[^>]*>([0-5][.,]\\\\d)<\\/span>[^<]*<a[^>]*>/i) || html.match(/Оценка:\\\\s*([0-5][.,]\\\\d)/i) || html.match(/aria-label="Рейтинг ([0-5][.,]\\\\d) из 5/i) || html.match(/>([0-5][.,]\\\\d)<\\/span>/);',
      '      const cMatch = html.match(/>([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:отзыв|отзыва|отзывов)/i) || html.match(/aria-label="[^"]*?([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:отзыв|отзыва|отзывов)/i) || html.match(/<a[^>]*>([\\\\d\\\\s\\\\u00A0]+)\\\\s*(?:Google|отзыв)/i) || html.match(/<span>\\\\(([\\\\d\\\\s\\\\u00A0]+)\\\\)<\\/span>/i) || html.match(/\\\\(([\\\\d\\\\s\\\\u00A0]+)\\\\)/);',
      '      if (rMatch) rating = parseFloat(rMatch[1].replace(",", "."));',
