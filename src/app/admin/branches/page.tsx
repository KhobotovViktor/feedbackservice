...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      // Google SERP: specifically match "N \\u043e\\u0442\\u0437\\u044b\\u0432ов в Google" to avoid Yandex data',
      '      const gBlock = html.match(/(\\d[\\.,]\\d)[\\s\\S]{0,200}?(\\d[\\s\\d]*)\\s*\\u043e\\u0442\\u0437\\u044b\\u0432\\S*\\s+в\\s+Google/);',
      ...
      '        const cMatch = html.match(/(\\d+)\\s+Google\\s+reviews/) || html.match(/(\\d+)\\s*\\u043e\\u0442\\u0437\\u044b\\u0432/);',
      '        if (rMatch) rating = rMatch[1].replace(",", ".");',
      '        if (cMatch) count = cMatch[1].replace(/\\D/g, "");',
      '      }',
      '    } else if (service === "2gis") {',
      '      // 2GIS: primary source is og:description meta tag',
      '      const ogMatch = html.match(/content="[^"]*\\u041e\\u0446\\u0435\\u043d\\u043a\\u0430\\s*(\\d[\\.,]\\d)[^"]*?(\\d+)\\\s*\\\\u043e\\\\u0442\\\\u0437\\\\u044b\\\\u0432/);',
      '      if (ogMatch) { rating = ogMatch[1].replace(",", "."); count = ogMatch[2]; }',
      '      // Fallback: JSON-LD schema.org',
      '      if (!rating) {',
      '        const rMatch = html.match(/"ratingValue"\\s*:\\s*"?(\\d[\\.,]?\\d*)/);',
      '        const cMatch = html.match(/"reviewCount"\\s*:\\s*"?(\\d+)/);',
      '        if (rMatch) rating = rMatch[1].replace(",", ".");',
      '        if (cMatch) count = cMatch[1];',
      '      }',
      '      // Fallback 2: page text patterns',
      '      if (!rating) {',
      '        const rMatch2 = html.match(/>(\\d[\\.,]\\d)<\\/span>/);',
      '        const cMatch2 = html.match(/>(\\d+)\\s*\\u043e\\u0446\\u0435\\u043d\\S*/);',
