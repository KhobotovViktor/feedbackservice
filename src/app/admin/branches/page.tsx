...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    const scriptLines = [
      ...
      '      syncService(branch.id, "google", branch.googleSearch, API_URL, SYNC_API_KEY, SERPER_KEY, branch.name);',
      '    }',
      '    if (branch.dgis) {',
      ...
      '}',
      '',
      'function syncService(branchId, service, urlOrQuery, apiUrl, syncApiKey, serperKey, branchName) {',
      ...
      '            // rating. Фолбэк — прямой fetch страницы Google Maps по CID',
      '            // с UA Googlebot. Для своих ботов Google отдаёт SSR-страницу',
      '            // с itemprop="ratingValue" и itemprop="reviewCount".',
      '            console.warn("❌ [GOOGLE] Карточка найдена, но без рейтинга: " + place.title + ". Пробую прямой fetch по CID…");',
      '            try {',
      '              const directOpts = {',
      '                muteHttpExceptions: true,',
      ...
      '              };',
      '              const directResp = UrlFetchApp.fetch("https://www.google.com/maps?cid=" + targetCid, directOpts);',
      '              if (directResp.getResponseCode() === 200) {',
      '                const html = directResp.getContentText();',
      '                const rMatch = html.match(/itemprop="ratingValue"\\s+content="([\\d.,]+)"/i) || html.match(/"ratingValue"\\s*:\\s*"?([\\d.,]+)"?/i) || html.match(/aggregateRating[^}]*?"ratingValue"\\s*:\\s*"?([\\d.,]+)"?/i);',
      '                const cMatch = html.match(/itemprop="reviewCount"\\s+content="(\\d+)"/i) || html.match(/"reviewCount"\\s*:\\s*"?(\\d+)"?/i) || html.match(/aggregateRating[^}]*?"reviewCount"\\s*:\\s*"?(\\d+)"?/i);',
      '                if (rMatch && cMatch) {',
      ...
      '                } else {',
      '                  console.warn("❌ [GOOGLE] Фолбэк: regex не нашёл rating/count на странице карточки (haveR=" + !!rMatch + ", haveC=" + !!cMatch + ")");',
      '                }',
      '              } else {',
